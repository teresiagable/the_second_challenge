...
public class WalletController {
	...
	public ResponseEntity<List<PlayerAccount>> getAllPlayerAccounts() {
		List<PlayerAccount> accounts = service.getAllPlayerAccounts();

		return accounts.isEmpty() ? ResponseEntity.noContent().build() : ResponseEntity.ok(accounts);

	}
	/**
	 * @GetMapping("/playeraccounts/{id}")
	 */
	@GetMapping("/playeraccounts/{id}")
	public ResponseEntity<PlayerAccount> getPlayerAccount(@PathVariable int id) {
		try {
			return ResponseEntity.ok(service.findPlayerById(id));
	...
	public ResponseEntity<PlayerTransaction> create(@RequestBody @Valid TransactionForm form) {
		...
		PlayerTransaction newTrans = service.creditDebit(form.getTransactionId(),
				service.findPlayerById(form.getPlayerId()), form.getAmount());
	...
	public ResponseEntity<PlayerAccount> createPlayer(@RequestBody @Valid PlayerAccountForm form) {
		...
		PlayerAccount newPlayer = service.createAccount(new PlayerAccount(form.getName()));
